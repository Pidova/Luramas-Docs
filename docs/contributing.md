...
Building instructions can be found [Building Luramas](building.md)
...
* **Passes are pure transforms.** A pass mutates only through the **pass_manager** API and reports whether it changed anything. Never touch the statement vector directly. See **[Writing a Pass](./Intermediate-Representation/writing-passes.md)**.

## Adding a target

To add a new target architecture:

1. Write a decoder under `framework/disassembler/<target>/` with an opcode table. See **[Disassemblers](./Intermediate-Language/Lifting/disassemblers.md)**.
2. Write a lifter under `framework/il/lifter/langs/<target>/` that emits **IL disassembly** into an **ilang** buffer. See **[Writing a Lifter](./Intermediate-Language/Lifting/writing-lifters.md)**.
3. Nothing in the IR or any pass changes. The middle is already written.

For a **native** target there is one extra front-end step: describe the instruction stream with the **[profile](./Intermediate-Language/Lifting/native-code.md)** block-builder before disassembling and lifting. 
...
When in doubt, **do nothing**. It is better to be correct then incorrect. See **[Passes: pass safety](./Intermediate-Representation/passes.md#pass-safety)**.
